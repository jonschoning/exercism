...
main :: IO ()
main = exitProperly $ runTestTT $ TestList [TestList matrixTests]
...
matrixTests =
  ...
  , testCase "shape" $ do
      ...
      (1, 2) @=? shape (msInt "1 2")
      (2, 2) @=? shape (msInt "1 2\n3 4")
  , testCase "rows & cols" $ (1, 2) @=? (rows &&& cols) (msInt "1 2")
  , testCase "eq" $ msInt "1 2" @=? msInt "1 2"
  ...
  , testCase "flatten" $ do v [1, 2, 3, 4] @=? flatten (msInt "1 2\n3 4")
