...
main :: IO ()
main = exitProperly $ runTestTT $ TestList [TestList bstTests]
...
bstTests :: [Test]
bstTests =
  [ testCase "data is retained" $ 4 @=? bstValue (singleton int4)
  ...
  , testCase "iterating one element" $ [4] @=? toList (singleton int4)
