...
main :: IO ()
main = exitProperly $ runTestTT $ TestList [TestList schoolTests]
...
schoolTests =
  ...
  , testCase "add more students in same class" $
    [(2, ["Blair", "James", "Paul"])] @=?
    S.sorted (gradeWithStudents 2 ["James", "Blair", "Paul"])
  , testCase "add students to different grades" $
    [(3, ["Chelsea"]), (7, ["Logan"])] @=?
    S.sorted (schoolFromList [(3, "Chelsea"), (7, "Logan")])
  , testCase "get students in a grade" $
    ["Bradley", "Franklin"] @=?
    S.grade 5 (schoolFromList [(5, "Franklin"), (5, "Bradley"), (1, "Jeff")])
  , testCase "get students in a non-existent grade" $ [] @=? S.grade 1 S.empty
